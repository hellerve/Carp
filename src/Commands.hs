...
commandCallableName :: UnaryCommandCallback
commandCallableName ctx xobj =
  pure (ctx, Right (fromMaybe (XObj (Lst []) Nothing Nothing) (extractName xobj)))
  where
    extractName (XObj (Lst (_ : XObj (Sym p mode) i t : _)) _ _) =
      Just (XObj (Sym p mode) i t)
    extractName (XObj (Closure (XObj (VMClosure _ (Just p) _ _) _ _) _) i t) =
      Just (XObj (Sym p Symbol) i t)
    extractName _ = Nothing

commandCallableArgs :: UnaryCommandCallback
commandCallableArgs ctx xobj =
  pure (ctx, Right (fromMaybe (XObj (Lst []) Nothing Nothing) (extractArgs xobj)))
  where
    extractArgs (XObj (Lst (_ : _ : args@(XObj (Arr _) _ _) : _)) _ _) = Just args
    extractArgs (XObj (Closure (XObj (VMClosure _ _ proper rest) i t) _) _ _) =
      let properArgs = map (\name -> XObj (Sym (SymPath [] name) Symbol) Nothing Nothing) proper
          allArgs = maybe properArgs (\r -> properArgs ++ [XObj (Sym (SymPath [] ":rest") Symbol) Nothing Nothing, XObj (Sym (SymPath [] r) Symbol) Nothing Nothing]) rest
       in Just (XObj (Arr allArgs) i t)
